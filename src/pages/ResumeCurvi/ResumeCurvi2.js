                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" id='resumeCurvi2' xmlnsXlink="http://www.w3.org/1999/xlink" version="1.1" width="595.5pt" height="842.25pt" viewBox="0 0 595.5 842.25">
                        <g enableBackground="new">
                            ...
                            <g clip-path="url(#cp2)">
                                ...
                                <text fill="#ffffff" xmlSpace="preserve" transform="matrix(.58659067 0 0 .58659067 55.577539 95.60123)" fontSize="37.63" fontFamily="Roboto">{user.name.toUpperCase().substring(0,17)}</text>
                                ...
                                <text fill="#323232" xmlSpace="preserve" transform="matrix(.7301169 0 0 .7301169 309.52897 297.1321)" fontSize="12.62" fontWeight='bolder' fontFamily="Roboto">{user.companyOccupation !== "NOT_PRINT" ? `${user.companyOccupation} |` : <></>}  {user.companyStartEnd !== 'NOT_PRINT' ? user.companyStartEnd : <></>}</text> 
                                <text fill="#323232" xmlSpace="preserve" transform="matrix(.7301169 0 0 .7301169 309.52897 317.1321)" fontSize="12.62" fontFamily="Roboto">{user.companyName !== 'Primeiro emprego objetivando adquirir conhecimento e experiência necessária junto à empresa.' && user.companyStartEnd !== "NOT_PRINT"  ? `${user.companyName} | ${user.companyStartEnd}` : user.companyName.substring(0,50)}</text> 
                                <text fill="#323232" xmlSpace="preserve" transform="matrix(.7301169 0 0 .7301169 309.52897 337.1321)" fontSize="12.62" fontFamily="Roboto">{user.companyName !== 'Primeiro emprego objetivando adquirir conhecimento e experiência necessária junto à empresa.' && user.companyStartEnd !== "NOT_PRINT"  ? `` : user.companyName.substring(50,user.companyName.length)}</text> 
                                ...
                                <text fill="#323232" xmlSpace="preserve" transform="matrix(.7301169 0 0 .7301169 304.31416 516.52424)" fontSize="12.07" fontWeight='bold' fontFamily="Roboto">{user.courseName} | {user.courseEndYear === 'Completo'?user.courseEndYear:`Previsão de Conclusão: ${user.courseEndYear}`}</text>
